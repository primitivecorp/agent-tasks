...
% Phase 2 [spec] keeps the one reconciler and adds a second \fold input:
% observations (a deployment landed, a review or approval was given, a step
% was done by hand, the ticket changed), written by connectors as
% AgentObservation objects and folded in sequence order exactly like step
% results. Sensors -> status -> reconcile -> actuators; the operator never
% calls an external system, and connectors never decide
