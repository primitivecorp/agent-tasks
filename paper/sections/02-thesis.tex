...
% subsequent task forever. Findings become tests (see Section 12). Give the
% compounding its concrete mechanism: a finding becomes a gate class, and the
% cluster policy can require that class in every plan it admits from then on.
% This is how trustworthiness compounds and N scales.
...
% Be explicit about status: the implementation ships Fail and manual merge
% [impl]; Retry, Elicit and the auto-merge policies are specified [spec] and
% land with the tracker connectors and the post-merge loop. Do not describe the
% dial as if every position were already turnable.
...
% why the substitution is economically viable at all. The invalidation default
% (Section 6.2) deliberately spends more of it for safety. Forward-reference 8.6.
