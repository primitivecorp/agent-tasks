...
% Mechanically these are environment-scoped gates (implementation.md
% section 12.4): keyed on (gate, environment, snapshot), runnable only once a
% deployment containing the snapshot is observed, Waiting otherwise, bounded
% by maxWait. Merge policy is an injected approval gate before the merge
% action (section 12.5): Manual injects it, AutoOnGreen does not.
