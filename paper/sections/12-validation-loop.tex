...
% Production verification is a workflow step, not an afterthought [spec]: the
% same gates run once more against the released system (smoke checks, health,
% the behaviour the ticket named), and a failure there re-opens the loop with
% the finding attached, exactly as a staging failure does.
% Mechanically these are environment-scoped or Observed gates
% (implementation.md section 12.4): keyed on (gate, environment, snapshot),
% runnable only once a deployment containing the snapshot is observed, Waiting
% otherwise, bounded by maxWait. How a team merges, deploys and releases is
% its own workflow and connectors (section 12.7 shows a lane, a merge queue,
% flags and previews from the same kinds); an administrator's floor is an
% approval gate injected before any action class (section 12.5).
